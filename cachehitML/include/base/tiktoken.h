...
static auto _byte_pair_merge(
  ...
  auto get_rank = [&piece, &ranks](
                      ...
                      int start_idx, int skip) -> std::optional<int> {
...
class tiktoken {
 public:
  ...
  tiktoken(ankerl::unordered_dense::map<std::string, int> encoder,
    ...
    // assert(encoder_.size() != decoder_.size() && "Encoder and decoder must be
    // of equal length; maybe you had duplicate token indices in your
    // encoder?");
 ...
 private:
  ...
  auto _encode_ordinary_native(const std::string &text) const
      -> std::vector<int> {
    std::vector<int> ret;
  ...
  auto _encode_native(const std::string &text,
    ...
    while (true) {
      auto [special, sub_input] =
          split_with_allowed_special_token(input, allowed_special);
      std::string piece;
