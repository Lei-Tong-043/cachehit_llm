...
//     switch (data_type){
//         case ML::DataType::DATA_TYPE_FP32:
//         case ML::DataType::DATA_TYPE_INT32:
//             return 4;
//         case ML::DataType::DATA_TYPE_INT8:
//             return 1;
//         default:
...
// // 1d tensor
// Tensor::Tensor(ML::DataType data_type,
//                int32_t dim0,
//                bool need_alloc,
//                std::shared_ptr<ML::DeviceAllocator> alloc,
//                void* ptr)
...
//     } else {
//         if (ptr != nullptr) {
//             CHECK(need_alloc == false) << "The need_alloc is true when ptr is not nullptr.";
...
//                void *ptr)
//     : data_type_(data_type)
// {   // init dim info and size
//     dims_.push_back(dim0);
//     dims_.push_back(dim1);
//     dims_.push_back(dim2);
//     dims_.push_back(dim3);
//     size_ = dim0 * dim1 * dim2 * dim3;
//     // if has buffer and need_alloc  
//     if (need_alloc && alloc){
//         allocate(alloc);
//     }else{
//         init_buffer(alloc, data_type_, need_alloc, ptr);
...
// {   // init dim info and size
//     size_ = reduce_dimension(dims_.begin(), dims.end(), 1);
//     // if has buffer and need_alloc  
//     if (need_alloc && alloc){
//         allocate(alloc);
//     }else{
...
//   if (device_type == ML::DeviceType::DEVICE_UNKNOWN) {
//     LOG(ERROR) << "The device type of the tensor is unknown.";
//   } else if (device_type == ML::DeviceType::DEVICE_CPU) {
//     size_t byte_size = this->byte_size();
//     auto cu_alloc = ML::CUDAAllocator::get_instance();
...
//   if (device_type == ML::DeviceType::DEVICE_UNKNOWN) {
//     LOG(ERROR) << "The device type of the tensor is unknown.";
//   } else if (device_type == ML::DeviceType::DEVICE_CUDA) {
//     size_t byte_size = this->byte_size();
//     auto cpu_alloc = ML::CPUAllocator::get_instance();
...
//   if (!buffer_) {
//     return ML::DeviceType::DEVICE_UNKNOWN;
//   }
...
//   if (!buffer) {
//     LOG(ERROR) << "The buffer parameter in the assign function is nullptr.";
//     return false;
//   }
  
//   if (buffer_) {
//     if (buffer->device_type() != buffer_->device_type()) {
...
//     return false;  // Return false here if size mismatch occurs
//   }
...
// {
//   if (!allocator){
//     LOG(ERROR) << "The allocator parameter in the allocate function is nullptr";
//     return false;
//   }
...
//   buffer_ = std::make_shared<ML::Buffer>(byte_size, allocator, nullptr);
//   if (!buffer_->ptr()){
//     LOG(ERROR) << "The memory allocated is a nullptr";
//     return false;
//   }
//   return true;
// }
...
// void Tensor::reset(ML::Datatype data_type,
...
// ML::DataType Tensor::data_type() const{
//   return data_type_;
// }
...
//         size * ML::DataTypeSize(this->data_type_), buffer_->allocator();
//     );
...
// std::shared_ptr<ML::Buffer> Tensor::get_buffer() const{
//   return buffer_;
// }
...
//         std::make_shared<ML::Buffer>(data_type_size(data_type) * size_,
//                                      nullptr, ptr, true);
//     this->buffer_ = buffer;
//   }else{
//     allocate(alloc, true);
