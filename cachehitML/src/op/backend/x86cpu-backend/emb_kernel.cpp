...
void emb_kernel_cpu(const tensor::Tensor& input, const tensor::Tensor& weight,
  ...
  CHECK(input.device_type() == cachehitML::DeviceType::kDeviceCPU);

  const auto allocator = cachehitML::CPUAllocatorFactory::get_instance();
  for (int32_t i = 0; i < input_num; ++i) {
    int32_t token = *input.ptr<int32_t>(i);
    if (token > vocab_size) {
      LOG(FATAL) << "Token index is lager than vocab size";
    } else {
      ...
      if (weight.device_type() == cachehitML::DeviceType::kDeviceCPU) {
        allocator->ML_memcpy(src_ptr, dest_ptr, weight_dim * sizeof(float),
                             cachehitML::MemcpyKind::kMemcpyCPU2CPU);
      } else {
